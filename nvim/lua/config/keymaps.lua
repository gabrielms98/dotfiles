local opts = { noremap = true, silent = true }

vim.keymap.set("n", "<leader>lg", "<cmd>silent !bash ~/.config/tmux-lazygit.sh<CR>")
vim.keymap.set("n", "<leader>ldc", "<cmd>silent !bash ~/.config/tmux-lazydocker.sh<CR>")
-- vim.keymap.set("n", "<leader>cu", "<cmd>silent !bash ~/.config/tmux-cursor-agent.sh<CR>")
vim.keymap.set("n", "<leader>cl", "<cmd>silent !bash ~/.config/tmux-claude-code.sh<CR>")
vim.keymap.set("n", "<leader>op", "<cmd>silent !bash ~/.config/tmux-opencode-agent.sh<CR>")
vim.keymap.set("n", "J", "mzJ`z")
vim.keymap.set("v", "J", ":m '>+1<CR>gv=gv")
vim.keymap.set("v", "K", ":m '<-2<CR>gv=gv")
vim.keymap.set("n", "<C-d>", "<C-d>zz", opts)
vim.keymap.set("n", "<C-u>", "<C-u>zz", opts)
vim.keymap.set("n", "n", "nzzzv")
vim.keymap.set("n", "N", "Nzzzv")
vim.keymap.set("i", "<C-c>", "<Esc>")
vim.keymap.set("n", "<C-b>", "<cmd>silent !tmux neww ~/.config/tmux-finder.sh<CR>")
vim.keymap.set("n", "<C-\\>", "<cmd>lua require('config.terminal').toggle()<CR>", { noremap = true, silent = true })
vim.keymap.set("n", "<leader>ac", "<cmd>lua AngularComponentSnippet()<CR>", { desc = "Angular Component Snippet" })
vim.keymap.set("n", "<leader>Y", function()
  local file_path = vim.fn.expand("%:p")
  vim.fn.setreg("+", file_path)
  print("Copied file path: " .. file_path)
end, { desc = "Copy File Path" })
vim.keymap.set("n", "<leader>y", function()
  local file_path = vim.fn.expand("%:p")
  local workspace_root = vim.fn.getcwd()
  local relative_path = vim.fn.fnamemodify(file_path, ":~:.")
  vim.fn.setreg("+", relative_path)
  print("Copied relative file path: " .. relative_path)
end, { desc = "Copy Relative File Path" })
